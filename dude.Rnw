...
\title{Dude, Where's my Alpha?\\dispatches from a `quant'}%FOLDUP
\author{Steven E. Pav \\
	Cerebellum Capital \\
	steven@cerebellumcapital.com}
%\date{\today}
\date{\Sexpr{compile.time}}
%UNFOLD
...
Later amended to include 'risk-free' rate:
...
Perhaps better to call it `Student Ratio'.
%\end{itemize}
%\begin{itemize}
...
\item Vaguely, in equities space, achieved SR translates as:
$1\yrto{-1/2} = \mbox{good}$,
$2\yrto{-1/2} = \mbox{excellent}$,
$3\yrto{-1/2} = \mbox{legend}$.\\
