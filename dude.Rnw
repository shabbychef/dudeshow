...
%\date{\today}
%\date[USF 2013]{\Sexpr{format(compile.time,'%Y-%m-%d')}}
\date[\Sexpr{format(compile.time,'%Y-%m-%d')}]{USF \Sexpr{format(compile.time,'%Y-%m-%d')}}
...
<<'wyrather',include=FALSE,fig=TRUE,out.width='6cm',out.height='4cm'>>=
nyr <- 5
epy <- 253
...
%\begin{frame}[allowframebreaks]{Basic Performance Analysis}
\begin{frame}[allowframebreaks]{Returns}%FOLDUP
\begin{itemize}
\item Let \pryp[t] be the `mark-to-market' (MtM) at time $t$.
\item Compute the \emph{returns}:
\begin{align*}
\mbox{Geometric:}\,\,\retg[t] &\defeq \log\frac{\pryp[t]}{\pryp[t-1]},\\
\mbox{Arithmetic:}\,\,\reta[t] &\defeq \frac{\pryp[t]}{\pryp[t-1]} - 1 = \exp{\retg[t]} - 1.
\end{align*}
...
\item By simple calculus: $\retg[t] \le \reta[t]$ with equality at zero.
\end{itemize}

% geometric/arithmetic%FOLDUP

<<'map_returns',include=FALSE,fig=TRUE>>=

# just get the TEOs
rvals <- seq(from=-0.50,to=0.50,by=0.01)
lvals <- rr2lr(rvals)

xat <- seq(from=-0.40,to=0.40,by=0.20)
tlab <- sapply(xat,function(dd) { sprintf('%g %%',100 * dd) })

plot(rvals,lvals,type='l',col='blue',
		 xaxt="n",
		 xlab='arithmetic return',ylab='geometric return')
...
\begin{figure}[htbp]
  \begin{center}
		\includegraphics[width=2.0in]{figure/map_returns}
		%\caption{the caption}
		\label{fig:map_returns}
	\end{center}
\end{figure}
\vspace{-0.20in}
%UNFOLD
...
\begin{frame}{It's a Tough World}%FOLDUP
\begin{itemize}
\item The opposite of a losing strategy might be a losing strategy!\\
Suppose Bob sells when Alice buys, and
vice versa. Because of compounding, they can both have negative total
log return. 
\item This effect is exacerbated by volatility.
\item Factoring in costs, the 'average' strategy is certainly a loser.
\end{itemize}
	
% Alice/Bob%FOLDUP

<<'alice_bob',include=FALSE,fig=TRUE>>=

# just get the TEOs
start.TEO <- "2000-01-01"
end.TEO <- "2010-01-01"
full.TEO <- paste(start.TEO,end.TEO,sep="::")
example.ts <- SPX.adjclose[full.TEO]

n.day <- dim(example.ts)[1]

# set up population
set.seed(171713)

alice.rr <- 0.0160 * rnorm(n.day)
bob.rr <- - alice.rr

alice.mtm <- rr2mtm(alice.rr)
bob.mtm <- rr2mtm(bob.rr)

alice.xt <- xts(data.frame("alice"=alice.mtm),order.by=time(example.ts))
bob.xt <- xts(data.frame("bob"=bob.mtm),order.by=time(example.ts))
both.xt <- merge(alice.xt,bob.xt)

par(mfrow=c(1,1))
plot.zoo(both.xt,screens=c(1,1),col=c('red','blue'),
				 xlab="time",ylab="mtm",
				 type=c('l','l'),lty=c('solid','solid'))

#locator(1)
legend(13800,1.3,legend=c('Alice','Bob'),col=c('red','blue'),
				 lty=c('solid','solid'))
...
\begin{figure}[htbp]
  \begin{center}
		\includegraphics[width=2.0in]{figure/alice_bob}
		%\caption{the caption}
		\label{fig:alice_bob}
	\end{center}
\end{figure}
...
\begin{frame}{the Risk Return Tradeoff}%FOLDUP
\begin{itemize}
\item Let $\pmu = \E{\retg[]}, \psigsq = \VAR{\retg[]}$.\\
Throughout, mostly assume \retg[t] are \iid (unrealistic).
...
\item Sharpe is often given in annualized terms, \ie \yrto{-1/2}.\\
...
\begin{align*}
\psnr = \Sexpr{srs[1]}\yrto{-1/2} &\Rightarrow \Pr{\mbox{down year}} \approx \Sexpr{signif(100 * prs[1],digits=2)}\mbox{\%}\\
\psnr = \Sexpr{srs[2]}\yrto{-1/2} &\Rightarrow \Pr{\mbox{down year}} \approx \Sexpr{signif(100 * prs[2],digits=2)}\mbox{\%}\\
\psnr = \Sexpr{srs[3]}\yrto{-1/2} &\Rightarrow \Pr{\mbox{down year}} \approx \Sexpr{signif(100 * prs[3],digits=2)}\mbox{\%}\\
\psnr = \Sexpr{srs[4]}\yrto{-1/2} &\Rightarrow \Pr{\mbox{down year}} \approx \Sexpr{signif(100 * prs[4],digits=2)}\mbox{\%}
\end{align*}
...
\item Under independence, \emph{Squared} SNR is subadditive.
For $k$ \coindie strategies, with SNRs $\psnr[1],\ldots,\psnr[k]$, optimal
rebalancing gives
$$\psrsq[*] = \sum_i \psrsq[i]$$
Diminishing returns in \psr : $2\yrto{-1/2} + 1\yrto{-1/2} \Rightarrow \Sexpr{signif(sqrt(2^2 + 1),digits=2)}\yrto{-1/2}$.
\item 'Fundamental Law': $\mbox{Sharpe} = \mbox{edge} \sqrt{\mbox{gambles}}.$
...
Avoid strategies correlated to `retail alpha'.
\end{itemize}
\end{frame}
%UNFOLD
...
\begin{itemize}
%\item Use \ssr to perform inference on \psr. It is actually a 
%\emph{biased} estimator: \cite{CambridgeJournals:4493808}
%\item SR is actually a \emph{biased} estimator of SNR:\cite{CambridgeJournals:4493808}
\item \ssr is a biased estimator of \psr: \cite{CambridgeJournals:4493808}
$$
...
\begin{alertblock}{rule of thumb for Sharpe Significance}
$\abs{\psr - \ssr} \le \frac{2}{\sqrt{\ssiz}}$, with probability $\approx 0.95$.
\end{alertblock}
\break
...
\item No easy fix for omitted variable bias \ie ``something changed in the world.''
\item Perhaps better to generalize to regression:
$$
\retg[t] = \alpha + \sum_i \pregco[i] \sfact[i,t] + \pserrt[t],
$$
with \sfact[i] returns from known benchmark strategies.
\item Typically want no exposure to \sfact[i] because it is
available via ETPs, 
or one believes they are zero mean or cannot be timed.
\item Whole literature on regression to perform inference on $\alpha$.
Inference on $\alpha/\psig$ is less common.
\end{itemize}
\end{frame}%UNFOLD
...
A power rule is the relation between sample size, effect size (SNR), and rates of 
...
colnames(foo) <- c("one sided","two sided")
xres <- xtable(foo,label="tab:ttestpower",
							 caption="Value of \\tpowc to achieve given power in t-test, $\\typeI = 0.05$.")
...
\begin{frame}[allowframebreaks]{Why do Bad Things Happen to Smart People?}%FOLDUP
\vspace{-0.35in}
\begin{figure}[htbp!]
  \begin{center}
...
\begin{itemize}
\item Bad backtests: inability to model costs and impact, observer effects, ``backtest arbitrage,''
outright ``time-travelling,'' survivorship \& backfill biases.
\item Biased statistical tests. Rejecting the null for uninteresting reasons.
\item Claims of ``non-stationarity'' (``the dog ate my alpha.'')
\end{itemize}
Perhaps most important:
\begin{itemize}
\item False positives / Excessive type I rate.
\item Sequential model overfit.
\end{itemize}

\end{frame}%UNFOLD

\begin{frame}{Setting Type I Rate}%FOLDUP
%How to set type I rate when \irate, the incidence rate of profitable
%strategies is unknown? Consider the outcomes:
...
\begin{tabular}{|l||c|c|}\hline
	%\rowcolor{gray}
	%\rowcolor{blue!10}
	test \textbackslash \, truth  & profitable  & not profitable \\\hline
	%\rowcolor{blue!10}
	reject null &  \cellcolor{blue!30} win! & \cellcolor{red!40} type I error \\
	\hline
	%\rowcolor{blue!10}
	fail to reject & \cellcolor{red!40} type II error & \cellcolor{blue!30} business as usual \\\hline
\end{tabular}
\end{center}
\vspace{0.02in}
Let \typeI, \typeII be the type I and II rates; let \irate be incidence rate of profitable
strategies. Condition on test rejection:
\vspace{0.02in}
\begin{center}
\begin{tabular}{|l||c|c|}\hline
	test \textbackslash \, truth & profitable ($\irate$) & not profitable ($1 - \irate$)\\\hline
	reject null &  \cellcolor{blue!30} $\irate (1-\typeII)$ & \cellcolor{red!40} $(1-\irate) \typeI$ \\
	\hline
	%\rowcolor{gray!90} 
...
\end{center}
False Discovery Rate (FDR) is ratio of false to all positives:
$$\mbox{FDR} \defeq \frac{(1-\irate)\typeI}{(1-\irate)\typeI + \irate(1-\typeII)} \ge 1 - \frac{\irate}{\typeI}\,\mbox{when}\,\typeII \ge \irate.$$

Probably $\irate \ll 0.05$. But is $\irate \approx 10^{-5}?$ $\approx 10^{-10}?$ 
\end{frame}%UNFOLD

\begin{frame}[allowframebreaks]{Why does Overfit Happen?}%FOLDUP
...
\break
Why do quants overfit? A toxic mix:
\begin{itemize}
\item Failure is not an option: cannot become an accountant.
\item The incidence rate is \emph{very} low.
\item By power rules, you need \emph{years} of data to confirm alpha.
\item There is only one history to work with:\\ 
One collects data at a rate of 1 day per day.
\item Recycling ideas developed (and overfit) by others.
\item Often `refining' ideas while debugging code.
%\item Cross-validation is a fiction: there is ``in-sample'' and ``trading with real money.''
\item Cross-validation won't help: there is only ``in-sample'' and ``trading with real money.''
%\item Human nature to confuse noise for signal.
\item Sloppy process: often not aware one is overfitting. 
\item Data snooping tests require good
record keeping, cannot deal with sequential overfit. 
\cite{White:2000,Hansen:2005,Hsu2010471}
\end{itemize}
\end{frame}%UNFOLD
%UNFOLD
...
\begin{frame}[allowframebreaks]{The Portfolio Problem}%FOLDUP
\begin{itemize}
\item Suppose you can invest in \nlatf different assets. (Stocks, ETPs, strategies, \etc)
How do you choose a portfolio?
\item This generalizes the discrete question, ``is this asset good or not?'' to
the continuous ``how much shall I invest in it?''
\item Consider the arithmetic returns \nlatf-vector, \vreta[t]. 
Let \pvmu, \pvsig be the (vector) mean and (matrix) covariance.
\item If \sportw is proportion of capital in each asset, return is $\tr{\sportw}\vreta[t]$.\\
This has mean $\tr{\sportw}\pvmu$ and variance \qform{\pvsig}{\sportw}.
The SNR is 
$$\frac{\tr{\sportw}\pvmu}{\sqrt{\qform{\pvsig}{\sportw}}}$$
In brief, pick a portfolio to maximize SNR.

\break

\item The population parameters are unknown. Given \ssiz observations of returns,
compute `usual' estimates \svmu, \svsig.
\item Markowitz Portfolio maximizes the SR: \cite{markowitz1952portfolio,markowitz1999early,markowitz2012foundations}
$$
\sportw[*]\defeq \argmax_{\sportw} \frac{\tr{\sportw}\svmu}{\sqrt{\qform{\svsig}{\sportw}}}.
$$
May also incorporate \rfr, but must bound risk: 
$$
\sportw[*]\defeq \argmax_{\sportw \,:\, \qform{\svsig}{\sportw} \le s^2}\, \frac{\tr{\sportw}\svmu - \rfr}{\sqrt{\qform{\svsig}{\sportw}}}.
$$
\item The optimal portfolio is
$\sportw[*]\propto\minv{\svsig}\svmu$.
% or \sqrt{\frac{s}{\qform{\minv{\svsig}}\svmu}} \minv{\svsig}\svmu
%\item The SR of this portfolio is
with SR:
$$
\ssr[*] = \sqrt{\qform{\minv{\svsig}}{\svmu}} - \frac{\rfr}{s}.
$$
%The RV $\wrapParens{\ssr[*] + \frac{\rfr}{s}}^2 = \qform{\minv{\svsig}}{\svmu}$ is familiar...
Drop the \rfr for now, and consider the RV
$\ssrsq[*] = \qform{\minv{\svsig}}{\svmu}$.

\end{itemize}

\end{frame}%UNFOLD

\begin{frame}{Markowitz \& Hotelling}%FOLDUP
...
%\end{equation}
%where \ssr[*] is SR of Markowitz portfolio $\sportw[*]\propto\minv{\svsig}\svmu$.\cite{Hotelling:1931,Sen20073741}  % Roy's UIT
\item Hotelling \Tstat is multivariate generalization of \tstat: \cite{Hotelling:1931,Sen20073741,timm2002applied}
$$
\Tstat \defeq \ssiz {\tr{\svmu}\svsig^{-1}\svmu} = \ssiz\ssrsq[*]
$$
(Recall that in the univariate case $\tstat = \sqrt{\ssiz}\ssr$.)
\item For normal returns, rescaled \Tstat has \flaw{}-distribution:
$$
\frac{(\ssiz - \nlatf)}{\nlatf (\ssiz - 1)} \ssiz \ssrsq[*] \sim \flaw{\ssiz\psnrsq[*],\nlatf,\ssiz - \nlatf},
$$
where \psnr[*] is SNR of \emph{population} optimal portfolio, $\pportw[*]=\minv{\pvsig}\pvmu$.\\
So \ssr[*] can be used to perform inference on \psnr[*].
\item \emph{n.b.} \psnr[*] is maximal SNR of \emph{any} portfolio, including \sportw[*].\\
We expect some loss: $\SNRfunc{\sportw[*]} \le \SNRfunc{\pportw[*]}$. The loss depends
on sample size, effect size; not well understood.
\end{itemize}
\end{frame}
...
\begin{itemize}
%\item Observe \ssr[*], perform inference on \psr[*], an upper bound
%on SNR of \sportw[*].
\item Observe \ssr[*], perform inference on $\psr[*] = \SNRfunc{\pportw[*]} \ge \SNRfunc{\sportw[*]}.$
...
(Oops! unbiased estimator of \psnrsq[*] can be negative!)
%\item CI, MLE on \psnr[*] via \flaw{}-CDF \& PDF.
%(\texttt{pf} and \texttt{df}).
%The `KRS estimator' .\cite{kubokawa1993estimation}
\item CI, MLE on \psnr[*] via \flaw{}-CDF \& PDF. Shrinkage estimators.\cite{kubokawa1993estimation}
%\item Via \flaw{}-distribution, if $\ssrsq[*] \le \frac{\arat}{1 - \arat}$ then MLE of $\psnr[*]$ is 0. \cite{MC1986216}
\item If $\ssrsq[*] \le \frac{\arat}{1 - \arat}$ then MLE of $\psnr[*]$ is 0. \cite{MC1986216}
%\item `Aspect ratio' $\arat\defeq\nlatf / \ssiz$ sets the bar for \ssrsq[*].
\end{itemize}
\vspace{-0.03in}
\begin{alertblock}{rule of thumb, based on \flaw{}-MLE}
``If $\ssrsq[*] < \arat$, don't bother!''
\end{alertblock}
\nocite{MC1986216}
\end{frame}
%UNFOLD
...
\begin{frame}{Example: Hotelling on S\&P Sector Indices}%FOLDUP
...
\end{itemize}
Conclusion: Markowitz portfolio on S\&P sectors \emph{not} recommended.\\
...
\\ Get time series of returns for each, $\vreti[1],\vreti[2],\ldots,\vreti[m]$.
...
<<'mac_example_SPY',include=FALSE,fig=TRUE>>=

library(TTR)

# test it on SPY
SPY.some <- SPX.adjclose 
SPY.rets <- rel.returns(SPY.some)
start.TEO <- "1999-01-01"
next.TEO <- "2000-01-01"
end.TEO <- "2011-01-01"
full.TEO <- paste(start.TEO,end.TEO,sep="::")
part.TEO <- paste(next.TEO,end.TEO,sep="::")

SPY.rets <- SPY.rets[full.TEO,]
SPY.open <- SPY.some[full.TEO,][[1]] 

win1 <- 50
win2 <- 170
SPY.mtm <- SPY.open * lr2mtm(SPY.rets)
sm1 <- SMA(SPY.mtm,win1)
sm2 <- SMA(SPY.mtm,win2)
sm.all <- merge(sm1,sm2,SPY.mtm)
sm.all <- sm.all[part.TEO,]
crossover <- sign(sm1 - sm2)
crossover <- crossover[part.TEO,]

# old way:
#par(mfrow=c(2,1),mar=c(2,2,1,1))
#plot.zoo(sm.all,screens=c(1,1,1),col=c('cyan','blue','black'),
#				 xlab="time",ylab="GSPC",
#				 type=c('l','l','l'),lty=c('solid','solid','solid'))
#				 #type=c('l','l','l'),lty=c('dashed','dashed','solid'))
#plot.zoo(crossover)

# new way
colrs <- c("red","blue","black")
ltys <- c("solid","solid","dotted")

# see http://rgraphics.limnology.wisc.edu/rmargins_layout.php
par(oma=c(2,2,2,2))
nf <- layout(matrix(c(1,2),2,1),widths=10,heights=c(7,3),TRUE)
# layout.show(nf)        
xls <- c(as.double(time(sm.all)[1]),100 + as.double(time(sm.all)[dim(sm.all)[1]]))
par(mar=c(2,4,1,1))
plot.zoo(sm.all,screens=c(1,1,1),col=colrs,
				 xlab="time",ylab="GSPC",
				 xlim=xls,
				 type=c('l','l','l'),lty=ltys)
#locator(1)

legend(14110,1585,legend=c(sprintf('%d day',win1),sprintf('%d day',win2),'GSPC'),
			 col=colrs,lty=ltys)

par(mar=c(1,4,1,1))
plot.zoo(crossover,xlim=xls,xlab="time",ylab=c("buy/sell"))
# done
...
\begin{figure}[htbp]
  \begin{center}
		\includegraphics[width=2.5in]{figure/mac_example_SPY}
		\caption{Two-window MAC on GSPC, with buy/sell signal at bottom.}
		\label{fig:mac_example_SPY}
	\end{center}
\end{figure}
...
Use $\funcit{\psnr}{\stratrc[*]} \lessapprox \max_{\mathcal{L}} \psnr = \psnr[*]$,
...
%Gives an upper bound on \SNRfunc{\stratrc[*]}.
\end{frame}
%UNFOLD
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{End Matter}%FOLDUP

\begin{frame}{Conclusions}%FOLDUP
\begin{itemize}
\item I hope I did not scare you.
\item Using statistics might prevent a quant disaster, but not 
a source of alpha \emph{per se}.
\item Process matters.
\item \emph{It is better to be lucky than good.}
\item We are looking for interns!
\end{itemize}
\end{frame}%UNFOLD
%UNFOLD
...
% all around good resources:
\nocite{abelson2012statistics,mallaby2010more}
...
n.x <- ifelse(FINAL.VERSION,24,16)
gen.per <- ifelse(FINAL.VERSION,2^18,2^15)

x.psis <- seq(0,2.5/sqrt(epy),length.out=n.x)
dprob <- 0.975
...
<<'ddown_plot',include=FALSE,fig=TRUE,cache=FALSE>>=
...
plot(x.psi.pa,y.mxv.pretty,
		 xlab="S.R. (yr^{-1/2})",ylab="max volatility (% yr^{-1/2})",
		 type='b',col='blue')
