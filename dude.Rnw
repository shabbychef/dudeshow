...
\begin{frame}{Basic Performance Analysis}%FOLDUP
\begin{itemize}
...
\end{align*}
(Perversely, Geometric returns are additive by telescoping.)
...
\item Since $\pmu,\psig$ unknown, must estimate them.
\end{itemize}
\end{frame}
%UNFOLD
%\begin{frame}[allowframebreaks]{the Sharpe Ratio}
...
(Log is a concave function so geometric SR likely to be lower.)
...
$1\yrto{-1/2} \Rightarrow \mbox{good}$,
$2\yrto{-1/2} \Rightarrow \mbox{excellent}$,
$3\yrto{-1/2} \Rightarrow \mbox{legend}$.\\
...
\item Often more natural to consider $\psrUL{2}{}, \ssrUL{2}{}$.
...
\begin{figure}[htbp]
  \begin{center}
		...
		\caption{Given two strategies with $\psnr[i] = \Sexpr{signif(snr1,digits=2)}\yrto{-1/2}$,
		\psnr[*] is plotted versus $\rho$.}
...
\begin{itemize}
\item SR is actually a biased estimator of SNR:\cite{CambridgeJournals:4493808}
$$
...
\begin{frame}{The Quant's Lament}
\vspace{-0.35in}
\begin{figure}[htbp!]
  \begin{center}
		\includegraphics[width=2.5in]{figure/Curve_fitting}
		\caption{Courtesy Automated Trader magazine.}
		\label{fig:overfit}
	\end{center}
\end{figure}
\vspace{-0.20in}
