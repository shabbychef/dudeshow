...
<<'wyrather',include=FALSE,fig=TRUE,dev=c("cairo_ps","pdf")>>=
nyr <- 5
epy <- 253
...
# force them to have the same long term mean:
sg1 <- 0.15
mu1 <- sr1 * sg1
sg2 <- mu1 / sr2
mu2 <- sr2 * sg2
...
legend(1 + 0.1*nyr,0.8 * max(max(m2),max(m1)), # the location
...
\begin{figure}[htbp]
  \begin{center}
		...
		\caption{Which would you rather invest in?}
		\label{fig:wyrather}
	\end{center}
\end{figure}
\vspace{-0.25in}
