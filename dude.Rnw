...
\item By simple calculus: $\grett[t] \le \arett[t]$ with equality at zero.
\end{itemize}
...
$$
\grett[t] = \alpha + \sum_i \pregco[i] \sfact[i,t] + \pserrt[t],
$$
...
\cite{White:2000,Hansen:2005,SSPA:2008}
\end{itemize}
\end{frame}%UNFOLD
%UNFOLD
...
\item If \sportw is proportion of capital in each asset, return is $\tr{\sportw}\vreti[t]$.\\
...
\item Statistics can useful to avoid quant disasters.
\item It is better to be lucky than good.
